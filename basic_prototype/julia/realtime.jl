...
function probe_mode(host::IPAddr, port::Integer, in_fpath::String, out_fpath::String)
    ...
    n_neurons = convert(UInt16, size(spks, 1))
    ...
    # Send header
    hdr_bytes = reinterpret(UInt8, UInt16[n_neurons])
    ...
    for i in 1:N_PTS
        ...
        filter_preds[i] = read(conn, UInt8)
